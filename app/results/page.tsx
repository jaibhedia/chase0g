...
export default function Results() {
  ...
  // Phase 3 — pull the on-chain leaderboard from the server (which reads the
  // ChaseLeaderboard contract). Fetch on mount, then once more shortly after, to catch
  // this match's just-posted entry. Stays empty/hidden when chain isn't configured.
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      try {
        ...
        if (!cancelled && Array.isArray(data?.rows)) setLeaderboard(data.rows);
      } catch { /* server offline → no leaderboard */ }
    };
    load();
  ...
  return (
    <main className="h-screen h-[100dvh] w-full flex flex-col overflow-y-auto px-solid-bg p-4 relative">
      ...
      <div className="w-full max-w-4xl mx-auto my-auto text-center space-y-8 p-4 relative z-10">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                {replay.chainTxHash && (
                  <a
                    href={`${OG_TX_EXPLORER}${replay.chainTxHash}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 inline-flex items-center justify-center gap-1.5 bg-[#4d2813] hover:brightness-110 pixel-border px-3 py-2 text-[#f4e7c3] text-xs uppercase tracking-wider transition-all"
                  >
                    <ExternalLink className="w-3.5 h-3.5" /> Leaderboard Tx
                  </a>
                )}
              </div>
            </div>
        ...
        {/* On-chain leaderboard (Phase 3 / 0G Chain). Each row's score is linked to its
            0G Storage replay root hash → trustlessly verifiable back to the AI transcript. */}
        {leaderboard.length > 0 && (
          ...
          >
            <h3 className="text-sm md:text-lg text-[#a78bfa] mb-1 uppercase tracking-wider flex items-center justify-center gap-2 pixel-shadow">
              <Link2 className="w-5 h-5" /> 0G Chain Leaderboard
            </h3>
            <p className="text-[#f4e7c3]/70 text-[11px] text-center mb-4">
              On-chain match history (Galileo testnet) — every score linked to its verifiable replay.
            </p>
            <div className="space-y-2">
              {leaderboard.map((row, i) => (
                <div
                  key={`${row.rootHash}-${i}`}
                  className="bg-[#261309] pixel-border p-3 flex items-center justify-between gap-3"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="px-2 py-1 bg-[#4d2813] text-[#f4e7c3] text-[10px] font-bold pixel-border shrink-0">
                      #{i + 1}
                    </span>
                    <span className="text-white text-sm font-bold truncate">{row.winner || 'Unknown'}</span>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="inline-flex items-center gap-1 text-yellow-300 text-sm font-black">
                      <Target className="w-4 h-4" /> {row.score}
                    </span>
                    {row.rootHash && (
                      <a
                        href={OG_STORAGE_SCAN}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[#9fb0d8] hover:text-[#5fcde4] text-[10px] font-mono inline-flex items-center gap-1 transition-colors"
                        title={row.rootHash}
                      >
                        {row.rootHash.slice(0, 8)}… <ExternalLink className="w-3 h-3" />
                      </a>
                    )}
                  </div>
                </div>
